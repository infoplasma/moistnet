...
class Pod():
    ...
    def generate_probe_xtab(self, pod_id=1, probe_id=1, days=30):
        ts = pd.Timestamp(datetime.now())
        td = ts - pd.Timedelta(days=days)
        n = self.get_pod_hourly_data(pod_id)
        n = n[td:ts]
    ...
    def display_pod_timeseries(self, pod_list=[1]):
        #fig = plt.figure()
        ...
        for i in range(4):
            for j in range(5):
                print(idx)
                if idx < len(pod_list):
                    data = self.get_pod_hourly_data(pod_list[idx])                 
    ...
    def display_pod_heatmap(self, pod_list=[1, 2], days=15):
        idx = 0 
        for i in pod_list:            
            n = self.get_pod_hourly_data(i)          
            plt.subplot(2, 2, idx + 1)
            plt.title('NODE {}'.format(i))
            sns.heatmap(self.generate_probe_xtab(i, 1, days), 
                    annot=True, vmin=0, vmax=100, cmap='YlGnBu')            
            plt.subplot(2, 2, idx + 2)
            plt.title('NODE {}'.format(i))
            sns.heatmap(self.generate_probe_xtab(i, 2, days), 
                    annot=True, vmin=0, vmax=100, cmap='YlGnBu')   
            idx = 2
        plt.show()

    def plot_mbt(self, pod_list=[1, 2], n_tail=cfg.N_SAMPLE):
        ...
        for j in range(len(pod_list)):
            ...
            # MOISTURE PLOT
            if idx < len(pod_list):
                data = self.get_pod_hourly_data(pod_list[idx]).iloc[-n_tail:]                 
